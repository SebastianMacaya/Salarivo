...
Actualización del 2026-09-09: `termination-estimate-v3` distingue `SIMPLE` (sueldo bruto explícito constante, predeterminado en la UI) y `DOCUMENTS` (reconstrucción documental, predeterminado en la API). Un total salarial extraído puede sugerir el importe de una simulación sin que la conciliación de todos los conceptos sea una condición para estimar; esa referencia conserva su origen y no confirma la base legal ni modifica los recibos. El neto orientativo aplica aportes editables sólo a sueldo y SAC, más retenciones ingresadas, sin motor fiscal ni tasa inferida desde el neto del recibo. Un saldo total de vacaciones reemplaza el proporcional. Los límites y las fuentes de estos supuestos quedan en el documento canónico de reglas.
