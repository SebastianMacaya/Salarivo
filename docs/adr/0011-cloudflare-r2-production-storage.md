...
- CORS admite sólo `PUT` desde `PUBLIC_ORIGIN`, con `Content-Type`, `If-Match`, `x-amz-meta-upload-session` y `x-amz-storage-class`; expone `ETag`. El lifecycle elimina objetos bajo `incoming/` y aborta multipart incompletos al día. No expira originales canónicos: su retención sigue la política persistida y el borrado durable del ADR 0009.
...
- Tests de arranque fail-closed para storage class, dominios públicos, CORS y lifecycle, más marcador, headers firmados, no reemisión sin ETag y reconciliación de carreras de copia.
