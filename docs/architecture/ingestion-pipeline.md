...
El parser 9 descarta unidades ubicadas antes de las columnas monetarias también en tablas que separan haberes remunerativos y no remunerativos. Esos números no completan un importe ausente ni reemplazan el haber de otra columna. El extractor conserva su versión 7 y puede reutilizar texto/OCR compatible. La migración 031 exige drenar workers y actualiza sólo jobs pendientes; las corridas, correcciones y aprobaciones de layouts anteriores conservan su versión. Reprocesar crea una corrida nueva sujeta a comparación y decisión de promoción. Un recibo anterior al parser 9 con totales explícitos balanceados y suma de haberes inconsistente puede solicitar una mejora aunque no conserve un issue previo; un intento con el fingerprint actual bloquea repeticiones. No se disparan OCR externos adicionales sólo por conceptos sin clasificación legal.

El parser 10 conserva la columna de cada haber de una tabla separada mediante `source_field`: `settlement.remunerativeAmount` o `settlement.nonRemunerativeAmount`. Mantiene el código de concepto y la descripción, incluidos SAC y reintegros; una fila con importes en ambas columnas produce dos renglones. Una grilla de haberes combinados no permite inferir esa separación. Los conceptos explícitos de sueldo mínimo conformado, básico y adicional de empresa/voluntario se normalizan con reglas literales; otros conceptos y ajustes negativos conservan revisión de su tratamiento para la simulación. Esta atribución entra en la comparación entre corridas y en el control de conflictos OCR: no se sustituye silenciosamente una clasificación ya activa. La migración 033 requiere drenar workers y cambia sólo el fingerprint pendiente y el default de nuevos layouts; las aprobaciones anteriores no se trasladan al parser 10. El reproceso reutiliza texto compatible y conserva correcciones humanas.
...
La clasificación de país precede a la detección de template, parser y OCR completo. El empleo con país confirmado provee el contexto; el país principal del perfil nunca determina la jurisdicción del documento. Sin ese contexto se usan señales explícitas y cerradas: identificador CUIT/CUIL en un recibo salarial, ARS/Argentina o un campo de país con código ISO válido. Idioma, signo `$` y el hecho de ejecutar el parser argentino no establecen país. Un recibo salarial reconocido sin evidencia de país puede conservar una extracción candidata, con `COUNTRY_UNCONFIRMED`, sin aplicar templates ni autoasociarlo.

`Document.countryCode`, procedencia, confianza y fecha forman un snapshot. La ingesta inicial, una asociación explícita o la confirmación del empleo pueden completar un snapshot ausente; un reproceso no lo reemplaza ni modifica el país del empleo. Una corrección explícita del país usa `USER_CONFIRMED`, conserva el valor anterior en `UserCorrection.document.countryCode` y no modifica las corridas históricas. Cada corrida conserva su propio contexto y las inconsistencias `COUNTRY_EMPLOYMENT_CONFLICT`/`COUNTRY_SNAPSHOT_CONFLICT`. Un país conocido fuera de AR genera `COUNTRY_NOT_SUPPORTED`, sin ejecutar el parser argentino ni OCR completo. Confirmar y asociar un empleo AR compatible, o corregir explícitamente el país documental, habilita un reproceso; el resultado nuevo conserva los issues anteriores para auditoría. La decisión humana del documento vence una detección contradictoria con un issue informativo `COUNTRY_DETECTION_OVERRIDDEN` que registra sólo los códigos ISO; nunca vence un país confirmado distinto en el empleo. Promoción y rollback rechazan corridas incompatibles con el país del documento o del empleo confirmado.

El registro de layouts agrega país, empleador, tipo `PAYROLL` y versión de parser a la identidad estructural. La búsqueda exige que todos coincidan y que el empleador esté verificado. Sólo AR/PAYROLL está habilitado; agregar metadata para otros países no habilita su extracción.
...
El Employer detectado queda en Document como procedencia aunque `employment_id` permanezca nulo. La autoasociación exige exactamente un Employment del owner, para ese Employer, país confirmado y moneda, cuyo intervalo cubra `payrollPeriod`. Cero o varias filas dejan el documento sin asociar. Cuando hay una única coincidencia, ImportBatchItem, Document y PayrollSettlement convergen transaccionalmente al mismo Employment.
