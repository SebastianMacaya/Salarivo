...
Si el cálculo no dispone de remuneración mensual, el simulador diferencia recibos ya analizados de documentos todavía no utilizables. Ofrece revisar el recibo más reciente con observaciones, consultar la lista por período o ingresar una remuneración sólo para simular. Cuando el último período tiene un único recibo normal y su remunerativo es positivo, permite copiarlo al campo como referencia: muestra origen, advierte sobre pagos extraordinarios y exige revisar y calcular explícitamente; no lo confirma como base ni modifica el recibo. Los avisos completos quedan desplegables; cuando existe resultado explican sus límites sin presentarse como bloqueos. La navegación conserva empleo y moneda y abre la observación del recibo, sin salarios en la URL. La prueba de jurisdicciones cubre ese recorrido y el paso de resultado no disponible a dos escenarios mediante una remuneración explícita, sin modificar documentos.
