...
El historial conserva datos completos: las series usan un gráfico desplazable dentro de su contenedor, selección de período y tabla exacta desplegable. Resumen e Historial muestran básico comparable, bruto habitual y neto habitual con una escala común; SAC y otros pagos no inflan las barras del sueldo habitual. Las perspectivas económicas usan los importes regulares proyectados disponibles, sin reemplazarlos por el total ni completar ausencias. La lectura de poder adquisitivo y la tabla económica comparan netos habituales; el comparador explícito agrega filas separadas para habitual, SAC y otros netos, conservando los totales y su explicación. El desglose visible del mes presenta bruto y neto de sueldo habitual, aguinaldo/SAC, otros pagos y total, en la perspectiva elegida. Si `regularMixed` identifica un recibo con sueldo y extras integrados, explica por qué el neto habitual queda sin discriminar. Los importes del mes y su desglose quedan fuera del área desplazable; hover y foco los actualizan y, al salir, vuelven al mes seleccionado o al último disponible. El modo privacidad oculta importes, variaciones salariales y la forma reconstruible del gráfico. Nombres de empresa, importes y metadata envuelven; el encabezado compacto del recibo tiene el nombre completo disponible en sus datos.
...
La lista de empleos usa filas compactas agrupadas en actuales, anteriores y continuidad por confirmar, por fecha de ingreso descendente dentro de cada grupo (favoritas e ID desempatan). Búsqueda por empresa, puesto, fechas o estado, insensible a tildes, y filtro por estado trabajan sobre los empleos ya autorizados en memoria. Conserva episodios simultáneos y reingresos separados por ID, sin combinar antigüedades, monedas ni recibos. Los filtros no se guardan en URL ni almacenamiento. A 1280px cada fila alinea empresa/fechas, hitos y salario; tablet y móvil apilan esos bloques. Nombres largos envuelven, cada fila abre su detalle y el menú conserva un control independiente de 44px.

Las filas y el detalle muestran antigüedad calendario y próximo aniversario a partir de las fechas registradas; la fecha actual se toma en Buenos Aires. Un empleo con fecha de salida muestra su duración hasta ese día, sin aniversario futuro. Fechas inválidas, futuras o una continuidad desconocida sin fecha final no producen hitos. Se muestran años y meses completos, o días durante el primer mes; los aniversarios del 29 de febrero se presentan el último día de febrero en años no bisiestos. Estos hitos describen el episodio laboral registrado, sin reemplazar la antigüedad del simulador legal. Cada fila agrega bruto y neto del último período con sueldo normal recurrente disponible, período y cobertura en períodos/documentos; ambos importes reutilizan el modo privacidad. No se persisten hitos ni se agregan consultas o notificaciones. La prueba browser cubre doce empleos, estados mixtos, reingresos, búsqueda, filtro/vacío, menú, detalle y privacidad entre 320 y 1440px.

Cada documento reciente abre directamente su revisión desde el nombre o el estado, conservando el empleo y un ID opaco en la URL. “Revisar nueva lectura” indica una propuesta pendiente: el visor explica la comparación y permite confirmar juntos los recibos del grupo compatible ya validado por servidor. Esa acción conserva ownership, versiones esperadas y precedencia humana, y es transaccional; cambios distintos se revisan por separado. Los borradores sin guardar bloquean la decisión y, después de confirmar, se puede avanzar al siguiente pendiente o volver al listado.
...
## Resumen mensual y preparación de un aumento

Resumen incorpora una tarjeta que consulta el comparador owner-only existente para los dos últimos períodos disponibles del contexto seleccionado. Identifica ambos períodos, incluso cuando no son meses consecutivos, separa el cambio del neto habitual del total cobrado y muestra los cambios identificados, la cobertura económica y su procedencia. La tarjeta se reinicia al cambiar contexto o períodos, cancela la consulta anterior y ofrece reintento ante error. Un único período invita a importar otro; un dato ausente o mixto no se convierte en cero. La conclusión y los resultados derivados respetan el modo privacidad.

“Preparar un aumento” es un panel plegable que opera sobre el historial ya autorizado, con aritmética exacta del paquete económico compartido. Permite elegir un mes base y usa su neto habitual ajustado al último IPC disponible como objetivo. El escenario aplica un porcentaje al último neto habitual registrado; muestra su período y el período del índice, sin presentarlo como salario actual ni como predicción de inflación, bruto o retenciones. Un recibo habitual más reciente sin neto utilizable impide sustituirlo silenciosamente por otro anterior. No incorpora endpoints, almacenamiento, notificaciones ni llamadas a IA. Los importes, porcentajes editables y conclusiones se ocultan en modo privacidad.

La prueba `npm run test:browser:salary-insights --workspace @salarivo/web` utiliza el harness local y datos sintéticos de los motores salariales/económicos para verificar cálculo, estados incompletos, navegación, cambios de contexto, privacidad y diseño responsive a 320/390/1440px. Bloquea una base salarial faltante o ambigua incluso si existe un neto agregado. Los placeholders de Resumen reservan también la tarjeta mensual y el panel de aumento; con la respuesta salarial demorada 2500ms, el pase local de carga midió CLS de 0,013 a 0,022 en Resumen a 320/390/1280px. La implementación y sus pruebas locales no acreditan despliegue productivo ni validación de retención con personas reales.
...
La sección `termination` es accesible desde empleos e historial. Prioriza el último empleo ACTIVE/DEPENDENT y ofrece confirmación para UNKNOWN. Fechas de ingreso/egreso, sueldo bruto habitual y vacaciones pendientes quedan visibles; los ajustes de aportes, retenciones, convenio y análisis documental son opcionales. La estimación simple usa el sueldo indicado constante. Cada escenario muestra bruto y neto orientativo juntos, con el desglose plegado; mobile apila las tarjetas y desktop compara columnas. Los datos utilizados, diagnósticos y fuentes quedan desplegables. Se reutilizan MoneyValue y privacidad visual, incluyendo campos de importe y porcentaje protegidos. Todo viaja por POST y permanece en memoria; no se guardan salarios en URL, localStorage ni cachés de PWA.
...
Al elegir empleo o fecha, el simulador consulta el endpoint existente para precargar la referencia del último recibo normal único elegible: total remunerativo positivo o, si falta, bruto positivo. Importe, período y procedencia corresponden al mismo recibo; no se reemplazan por un mes anterior conciliado. Esta consulta no presenta un resultado final. El botón Calcular envía el sueldo visible para estimar sin exigir revisión de conceptos; si no existe referencia, permite completarlo manualmente. Un cambio del formulario retira el resultado anterior, y la precarga no pisa un sueldo editado. El modo documental conserva sus diagnósticos y enlaces de revisión. Los días pendientes reemplazan el saldo automático, incluido cero. Los aportes se ajustan sólo sobre sueldo/SAC y el neto explica que no calcula Ganancias ni topes automáticamente. La prueba de jurisdicciones cubre estos recorridos con el motor real y recibos sintéticos ambiguos.
