...
No hay break-glass, impersonación, acceso al original, reproceso completo, cancelación de jobs `RUNNING`, retry de fallos permanentes, baja administrativa de cuenta, tickets, flags ni settings editables. Queue, storage, OCR y OAuth se muestran como `UNKNOWN` cuando no existe una señal segura y comprobable; la UI no inventa telemetría.

Agregar cualquiera de esas operaciones requiere preservar las máquinas de estado, definir la base de autorización, minimizar el DTO y dejar una prueba que cubra permiso, IDOR, concurrencia y auditoría. Ver [ADR 0012](../adr/0012-granular-admin-console.md) y [ADR 0014](../adr/0014-global-employer-resolution.md).
