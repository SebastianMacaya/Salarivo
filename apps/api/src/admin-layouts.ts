...
import {
  layoutAliasFields, lockEmployerMutation, pool, processingPipelineVersions, validateLayoutAliases, withTransaction, type LayoutAliases,
...
export async function registerAdminLayoutRoutes(app: FastifyInstance, dependencies: AdminRouteDependencies) {
  ...
  app.get<{ Querystring: { page?: number; pageSize?: number } }>("/api/v1/admin/processing/layouts", {
      ...
      response: { 200: { type: "object", additionalProperties: false, required: ["data"], properties: { data: {
        type: "object", additionalProperties: false, required: ["items", "page", "pageSize", "total"], properties: {
          ...
          items: { type: "array", items: {
            type: "object", additionalProperties: false,
            required: ["employerId", "fingerprint", "fingerprintVersion", "observedRuns", "lastSeenAt", "layoutId", "versions", "versionCount", "countryCode", "documentType", "parserVersion"],
            properties: { employerId: uuid, fingerprint, fingerprintVersion: { type: "string", const: "1" },
              countryCode: { type: 'string', const: 'AR' }, documentType: { type: 'string', const: 'PAYROLL' }, parserVersion: { type: 'string' },
    ...
    const observations = `WITH observed AS (
      SELECT run.detected_employer_id AS employer_id, run.layout_fingerprint_version AS fingerprint_version,
       ...
       WHERE run.layout_fingerprint IS NOT NULL AND run.status = 'REVIEW_REQUIRED'
         AND run.country_code = 'AR' AND document.document_type = 'PAYROLL' AND run.parser_version = '${processingPipelineVersions.parser}'
      ...
      SELECT COALESCE(layout.employer_id, observed.employer_id) AS employer_id,
        ...
        FROM (SELECT * FROM document_layouts WHERE country_code = 'AR' AND document_type = 'PAYROLL'
          AND parser_version = '${processingPipelineVersions.parser}') layout FULL JOIN observed ON layout.employer_id = observed.employer_id
    ...
    return { data: { page, pageSize, total: Number(count.rows[0]!.total), items: rows.rows.map((row) => ({
      ...
      countryCode: 'AR', documentType: 'PAYROLL', parserVersion: processingPipelineVersions.parser,
  ...
  app.post<{ Body: ApprovalBody }>("/api/v1/admin/processing/layouts/approve", {
    ...
    schema: {
      ...
      response: { 200: { type: "object", additionalProperties: false, required: ["data"], properties: { data: {
        ...versionSchema, required: [...versionSchema.required, "layoutId", "fingerprint", "fingerprintVersion", "countryCode", "documentType", "parserVersion"],
    ...
    return withTransaction(async (client) => {
      ...
      const existing = await client.query(`SELECT id FROM document_layouts WHERE employer_id = $1 AND fingerprint_version = $2
        AND structural_fingerprint = $3 AND country_code = 'AR' AND document_type = 'PAYROLL' AND parser_version = $4`,
      [request.body.employerId, request.body.fingerprintVersion, request.body.fingerprint, processingPipelineVersions.parser]);
      if (!existing.rowCount) {
        const observed = await client.query(`SELECT 1 FROM extraction_runs run
          ...
          WHERE run.detected_employer_id = $1 AND run.layout_fingerprint_version = $2 AND run.layout_fingerprint = $3
            AND run.country_code = 'AR' AND document.document_type = 'PAYROLL' AND run.parser_version = $4
      ...
      if (!existing.rowCount) await client.query(`INSERT INTO document_layouts (id, employer_id, fingerprint_version, structural_fingerprint,
        country_code, document_type, parser_version) VALUES ($1, $2, $3, $4, 'AR', 'PAYROLL', $5)`,
      [layoutId, request.body.employerId, request.body.fingerprintVersion, request.body.fingerprint, processingPipelineVersions.parser]);
