...
import { FormEvent, type KeyboardEvent, type MouseEvent, lazy, Suspense, useCallback, useEffect, useRef, useState } from 'react';
...
const DocumentReview = lazy(() => import('./document-review').then((module) => ({ default: module.DocumentReview })));
...
type PeriodComparison = {
  ...
  netBreakdown?: Record<'regular' | 'sac' | 'other', MoneyChange | null>;
...
const SESSION_EXPIRED_EVENT = 'salarivo:session-expired';
...
function notifyLegalAcceptanceRequired(code: string) {
  ...
  if (code === 'AUTHENTICATION_REQUIRED') window.dispatchEvent(new Event(SESSION_EXPIRED_EVENT));
}
...
export function SalarivoApp() {
  ...
  useEffect(() => {
    ...
    let checking = false;
    const expired = () => {
      setAccessError('Tu sesión finalizó. Ingresá nuevamente para continuar.');
      setUser(null);
    };
    const revalidate = async () => {
      ...
      try {
        ...
        if (!stopped) setUser(current);
      } catch { /* Sólo AUTHENTICATION_REQUIRED cierra la sesión; una caída de red no descarta formularios. */ }
...
function AccessScreen({ initialError, initialMode, onAuthenticated, onGoogleRegistrationCancelled, onReceiptToken }: {
  ...
  return (
    <main className="access-layout access-login">
      ...
      <section className="access-panel" aria-labelledby="access-title">
        <div className="access-card"><div className="mobile-only access-brand"><Brand /></div>
...
function PrivateApp({ user, authNotice, onAuthNoticeDismiss, onUserChanged, onLogout, onDeletionRequested }: {
  ...
  const navigate = useCallback<NavigateApp>((nextSection, options = {}) => {
    ...
    window.requestAnimationFrame(() => {
      document.getElementById('private-content')?.focus({ preventScroll: true });
  ...
  return (
    <div className="app-shell">
      <a className="skip-link" href="#private-content">Ir al contenido</a>
      <AppNavigation id="private-navigation" label="Navegación principal" className="sidebar" open={menuOpen} onClose={closeMenu}>
      ...
      <nav className="mobile-navigation" aria-label="Accesos principales" inert={sensitiveActions.open || undefined}>
        <button type="button" disabled={importBusy} aria-current={section === 'summary' ? 'page' : undefined} onClick={() => navigate('summary')}><span aria-hidden="true">⌂</span>Inicio</button>
        <button type="button" disabled={importBusy} aria-current={section === 'history' && ownerLocation.tab === 'documents' ? 'page' : undefined} onClick={() => navigate('history', { tab: 'documents' })}><span aria-hidden="true">▤</span>Recibos</button>
        <button type="button" disabled={importBusy} aria-current={section === 'import' ? 'page' : undefined} onClick={() => navigate('import')}><span aria-hidden="true">↑</span>Subir</button>
        <button type="button" disabled={importBusy} aria-current={section === 'history' && ownerLocation.tab !== 'documents' ? 'page' : undefined} onClick={() => navigate('history')}><span aria-hidden="true">≋</span>Historial</button>
        <button type="button" aria-label="Más: empleos y cuenta" aria-expanded={menuOpen} aria-controls="private-navigation" onClick={() => setMenuOpen(true)}><span aria-hidden="true">☰</span>Más</button>
      </nav>
      {sensitiveActions.open && <StepUpDialog
        mfaEnabled={Boolean(user.mfaEnabled)}
        onClose={() => sensitiveActions.finish(false)}
...
function StepUpDialog({ mfaEnabled, onClose, onComplete, returnFocus }: { mfaEnabled: boolean; onClose: () => void; onComplete: () => void; returnFocus: HTMLElement | null }) {
  ...
  return (
    <ResponsiveDialog labelId="step-up-title" busy={busy} onClose={onClose}>
...
function SalaryScopeControl({ history, employments = [], selectedKey, onChange, id }: {
  ...
  return <section className="scope-control" aria-label="Contexto salarial">
    <div className="scope-picker"><label htmlFor={`${id}-search`}>Empleo y moneda</label>{history.contexts.length > 1
      ...
      : <strong>{context.employerName || 'Empleo sin confirmar'} · {context.currencyCode}</strong>}</div>
...
function SalaryEvolution({ scope, year = 'all', limit, perspective = 'nominal', recoveryPeriods = new Map(), selectedPeriod, onSelectPeriod }: { scope: SalaryScopeAnalytics; year?: string; limit?: number; perspective?: EconomicPerspective; recoveryPeriods?: Map<string, SalaryRecoveryState>; selectedPeriod?: string; onSelectPeriod?: (period: string) => void }) {
  ...
  const valuesFor = (point: MonthlyEvolution) => {
    ...
    return {
      ...
      amounts: economic ? projection?.regularAmounts ?? null : point.sac && point.other ? point.regular : null,
      sac: economic ? projection?.sacAmounts ?? null : point.sac,
      other: economic ? projection?.otherAmounts ?? null : point.other,
      total: economic ? projection?.amounts ?? null : point.totals,
  ...
  const chartValues = chartPoints.flatMap((point) => { const values = valuesFor(point); return [values.comparableSalary, values.amounts?.grossAmount ?? null, values.amounts?.netAmount ?? null]; });
  const visualValues = chartValues
  ...
  const protectedHeights = privacyChartHeights(chartValues);
  ...
  const latestRealPoint = [...points].reverse().find((point) => point.economic?.purchasingPower.regularAmounts?.netAmount != null && typeof point.economic?.comparisonToPrevious?.purchasingPower.changeBasisPoints === 'string');
  const latestRealComparison = latestRealPoint?.economic?.comparisonToPrevious ?? null;
  const economicExactTable = <div className="table-wrap responsive-data salary-evolution-table economic-monthly-table" role="region" aria-label="Evolución económica por período salarial del neto" tabIndex={0}>
    <table role="table">
      <caption className="sr-only">Neto habitual nominal, equivalente histórico en USD, inflación y poder adquisitivo por período</caption>
      <thead><tr><th scope="col">Período</th><th scope="col">Neto habitual original</th><th scope="col">USD histórico</th><th scope="col">Inflación desde el período anterior</th><th scope="col">{referencePeriod ? `Neto a precios de ${periodLabel(referencePeriod)}` : 'Poder adquisitivo'}</th></tr></thead>
      <tbody>{points.map((point) => {
        const usd = point.economic?.historicalUsd;
        const purchasingPower = point.economic?.purchasingPower;
        const comparison = purchasingPower?.regularAmounts !== undefined ? point.economic?.comparisonToPrevious : undefined;
        ...
        return <tr className={selectedPeriod === point.period ? 'selected' : ''} key={point.period} onClick={() => choosePeriod(point.period)}>
          ...
          <td data-label="Neto habitual original"><MoneyValue value={point.sac && point.other ? point.regular.netAmount : null} currency={scope.currencyCode} kind="salary" />{point.regularMixed && <small className="nd-context">Sueldo y extras sin discriminar</small>}</td>
          <td data-label="USD histórico"><div className="economic-monthly-value"><MoneyValue value={usd?.regularAmounts?.netAmount} currency={usd?.currencyCode ?? 'USD'} kind="salary" />{fromPeriod && <small>Vs. {periodLabel(fromPeriod)}: <PercentageValue value={percentageFromBasisPoints(comparison?.historicalUsd.changeBasisPoints)} /></small>}<EconomicProjectionNote projection={usd} /><EconomicChangeNote change={comparison?.historicalUsd} /></div></td>
          ...
          <td data-label={referencePeriod ? `Neto a precios de ${periodLabel(referencePeriod)}` : 'Poder adquisitivo'}><div className="economic-monthly-value"><MoneyValue value={purchasingPower?.regularAmounts?.netAmount} currency={purchasingPower?.currencyCode ?? scope.currencyCode} kind="salary" />{fromPeriod && <small>Vs. {periodLabel(fromPeriod)}: <EconomicRealChange basisPoints={comparison?.purchasingPower.changeBasisPoints} /></small>}<EconomicProjectionNote projection={purchasingPower} /><EconomicChangeNote change={comparison?.purchasingPower} /></div></td>
        </tr>;
      })}</tbody>
    </table>
  </div>;
  ...
  return <div className="salary-evolution">
    ...
    {perspective === 'purchasing-power' && <div className="economic-reading" role="note"><strong>¿Tu sueldo habitual ganó o perdió poder de compra?</strong>{latestRealPoint && latestRealComparison ? <p>Último cambio real del sueldo habitual: <EconomicRealChange basisPoints={latestRealComparison.purchasingPower.changeBasisPoints} /> · {periodLabel(latestRealComparison.fromPeriod)} → {periodLabel(latestRealPoint.period)}.</p> : <p>Todavía no hay dos períodos con datos completos para medir el cambio real.</p>}<p>Comparamos el neto del sueldo habitual ajustado por IPC. El aguinaldo y los pagos extraordinarios se muestran en el desglose y no generan aumentos o caídas del sueldo por entrar o salir del mes.</p></div>}
    <div className="legend"><span className="comparable">Básico comparable</span><span className="gross">Bruto habitual</span><span className="net">Neto habitual</span></div>
    {visualValues.length > 0 && <>
      <div className="chart-reading"><strong>{periodLabel(previewPoint.period)}</strong><span>Básico: <MoneyValue value={previewValues.comparableSalary} currency={previewValues.currency} kind="salary" /></span><span>Bruto: <MoneyValue value={previewValues.amounts?.grossAmount} currency={previewValues.currency} kind="salary" /></span><span>Neto: <MoneyValue value={previewValues.amounts?.netAmount} currency={previewValues.currency} kind="salary" /></span></div>
      <div className="bar-chart salary-chart" role="group" aria-label={`Períodos del historial salarial en perspectiva ${perspectiveLabel}`} onMouseLeave={() => setPreviewPeriod(null)} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPreviewPeriod(null); }}>{chartPoints.map((point, pointIndex) => { const values = valuesFor(point); return <button type="button" className={`bar-group${selectedPeriod === point.period ? ' selected' : ''}`} aria-label={`Abrir detalle de ${periodLabel(point.period)}`} aria-pressed={selectedPeriod === point.period} disabled={!onSelectPeriod} onMouseEnter={() => setPreviewPeriod(point.period)} onFocus={() => setPreviewPeriod(point.period)} onClick={() => choosePeriod(point.period)} key={point.period}><span className="bars" aria-hidden="true">{values.comparableSalary !== null && <i className="bar comparable" style={{ height: chartHeight(values.comparableSalary, pointIndex * 3) }} />}{values.amounts?.grossAmount != null && <i className="bar gross" style={{ height: chartHeight(values.amounts.grossAmount, pointIndex * 3 + 1) }} />}{values.amounts?.netAmount != null && <i className="bar net" style={{ height: chartHeight(values.amounts.netAmount, pointIndex * 3 + 2) }} />}</span><small>{periodLabel(point.period)}</small></button>; })}</div>
    ...
    <section className="salary-payment-breakdown" aria-label={`Desglose de ${periodLabel(previewPoint.period)}`}>
      <h3>Desglose de {periodLabel(previewPoint.period)}</h3>
      <p className="coverage-note">{economic ? `Importes en perspectiva ${perspectiveLabel.toLocaleLowerCase('es-AR')}. ` : ''}Separamos los pagos según los recibos; el mes por sí solo no identifica un aguinaldo.</p>
      {previewPoint.regularMixed && <p className="message warning">Este mes incluye sueldo y pagos extraordinarios en un mismo recibo. Como no se pueden repartir sus descuentos con certeza, el neto habitual queda sin discriminar. El total se conserva.</p>}
      <div className="table-wrap responsive-data"><table><thead><tr><th scope="col">Liquidación</th><th scope="col">Bruto</th><th scope="col">Neto</th></tr></thead><tbody>{([
        ['Sueldo habitual', previewValues.amounts], ['Aguinaldo / SAC', previewValues.sac],
        [previewPoint.regularMixed ? 'Otros pagos / sin discriminar' : 'Otros pagos', previewValues.other], ['Total del mes', previewValues.total],
      ] as Array<[string, SalaryAmounts | null | undefined]>).map(([label, amounts]) => <tr key={label}><th scope="row">{label}</th><td data-label="Bruto"><MoneyValue value={amounts?.grossAmount} currency={previewValues.currency} kind="salary" /></td><td data-label="Neto"><MoneyValue value={amounts?.netAmount} currency={previewValues.currency} kind="salary" /></td></tr>)}</tbody></table></div>
    </section>
...
const comparisonAmountLabels: Array<[keyof PeriodComparison['changes'], string]> = [
  ...
  ['grossAmount', 'Bruto total'],
  ['netAmount', 'Neto total'],
...
function EconomicComparison({ economic }: { economic: NonNullable<PeriodComparison['economic']> }) {
  return <section className="economic-comparison" aria-labelledby="economic-comparison-title">
    <div><p className="eyebrow">Datos económicos</p><h4 id="economic-comparison-title">Poder de compra del sueldo habitual</h4><p>Comparamos el neto habitual sin agregar recibos de aguinaldo ni otros pagos extraordinarios. Los totales originales se conservan.</p></div>
...
function ComparisonResult({ comparison }: { comparison: PeriodComparison }) {
  ...
  const rows: Array<{ label: string; change?: MoneyChange | null; creditAware?: boolean }> = [
    { label: 'Neto habitual', change: comparison.netBreakdown?.regular },
    { label: 'Aguinaldo / SAC (neto)', change: comparison.netBreakdown?.sac },
    { label: 'Otros pagos (neto)', change: comparison.netBreakdown?.other },
    ...comparisonAmountLabels.map(([key, label]) => ({ label, change: comparison.changes[key], creditAware: key === 'deductionsAmount' })),
  ];
  return <div className="comparison-result" aria-live="polite">
    <p className="coverage-note">La fila Neto habitual compara tu sueldo. El aguinaldo y los otros pagos se discriminan para que su variación no se confunda con un aumento o una caída salarial.</p>
    <div className="table-wrap responsive-data" role="region" aria-label="Comparación de períodos" tabIndex={0}><table role="table"><caption className="sr-only">Comparación de períodos</caption><thead><tr><th scope="col">Importe</th><th scope="col">{periodLabel(comparison.fromPeriod)}</th><th scope="col">{periodLabel(comparison.toPeriod)}</th><th scope="col">Diferencia</th><th scope="col">Variación</th></tr></thead><tbody>{rows.map(({ label, change, creditAware }) => {
      return <tr key={label}><th scope="row">{label}</th><td data-label={periodLabel(comparison.fromPeriod)}><MoneyValue value={change?.fromAmount} currency={currency} kind="salary" creditAware={creditAware} /></td><td data-label={periodLabel(comparison.toPeriod)}><MoneyValue value={change?.toAmount} currency={currency} kind="salary" creditAware={creditAware} /></td><td data-label="Diferencia"><MoneyValue value={change?.deltaAmount} currency={currency} kind="salary" /></td><td data-label="Variación"><PercentageValue value={change?.percentage} /></td></tr>;
    })}</tbody></table></div>
    <p className="comparison-conclusion">Sobre el total cobrado: {comparison.conclusionCode ? comparisonConclusionLabels[comparison.conclusionCode] : 'Comparación calculada con los importes disponibles.'}</p>
    ...
    {comparison.earnings && comparison.earnings.length > 0 && <details><summary>Ver conceptos normalizados</summary><div className="table-wrap responsive-data" role="region" aria-label="Conceptos comparados" tabIndex={0}><table role="table"><thead><tr><th scope="col">Concepto</th><th scope="col">Antes</th><th scope="col">Después</th><th scope="col">Diferencia</th></tr></thead><tbody>{comparison.earnings.map(({ code, change }) => <tr key={code}><td data-label="Concepto">{earningLabels[code] ?? 'Otro concepto'}</td><td data-label="Antes"><MoneyValue value={change.fromAmount} currency={currency} kind="salary" /></td><td data-label="Después"><MoneyValue value={change.toAmount} currency={currency} kind="salary" /></td><td data-label="Diferencia"><MoneyValue value={change.deltaAmount} currency={currency} kind="salary" /></td></tr>)}</tbody></table></div></details>}
    {comparison.economic && comparison.netBreakdown && <EconomicComparison economic={comparison.economic} />}
  </div>;
}
...
function AnnualHistory({ rows, scope, category }: { rows: AnnualSalarySummary[]; scope: SalaryScopeAnalytics; category: 'all' | SalaryCategory }) {
  ...
  return <div className="annual-list">{rows.map((annual) => {
    ...
    return <details className="panel annual-card" key={annual.year}><summary><span><strong>{annual.year}</strong><small>{annual.periodCount} período{annual.periodCount === 1 ? '' : 's'} · {annual.documentCount} documento{annual.documentCount === 1 ? '' : 's'}</small></span><strong><MoneyValue value={annual.totals.netAmount} currency={scope.currencyCode} kind="salary" /></strong></summary><div className="annual-body">
      ...
      <div className="table-wrap responsive-data" role="region" aria-label={`Tabla desplazable del resumen ${annual.year}`} tabIndex={0}><table role="table"><thead><tr><th scope="col">Categoría</th><th scope="col">Liquidaciones</th><th scope="col">Bruto</th><th scope="col">Neto</th><th scope="col">Conceptos normalizados</th></tr></thead><tbody>{categories.map((item) => {
...
function Summary({ user, onNavigate }: { user: User; onNavigate: NavigateApp }) {
  ...
  return (
    <div className="page" aria-busy={historyLoading || documentsLoading}>
      ...
      {historyLoading && !history && <SummaryLoading />}
      ...
      <div className="dashboard-grid summary-documents-grid">
        {historyLoading && !history && <div className="panel chart-panel" aria-hidden="true"><div className="panel-heading"><div><p className="eyebrow">Evolución</p><h2>Sueldo habitual reciente</h2></div><span className="loading-skeleton loading-action">&nbsp;</span></div><div className="salary-evolution"><div className="legend"><span className="loading-skeleton loading-range">&nbsp;</span></div><div className="bar-chart loading-skeleton" /><div className="evolution-details loading-skeleton loading-action">&nbsp;</div></div></div>}
        {history && context && scope && <section className="panel chart-panel"><div className="panel-heading"><div><p className="eyebrow">Evolución</p><h2>Sueldo habitual reciente</h2></div><button className="text-button" onClick={() => onNavigate('history', { tab: 'summary', currencyCode: context.currencyCode, employmentContext: context.employmentContext, employmentId: context.employmentId })}>Analizar historial</button></div><SalaryEvolution scope={scope} limit={12} onSelectPeriod={(period) => onNavigate('history', { tab: 'evolution', currencyCode: context.currencyCode, employmentContext: context.employmentContext, employmentId: context.employmentId, period })} /></section>}
...
function EmploymentMilestones({ employment }: { employment: Employment }) {
  const milestones = employmentMilestones(employment);
  if (!milestones) return null;
  const { anniversary } = milestones;
  return <dl className="employment-milestones">
    <div><dt>{employment.endDate ? 'Duración del empleo' : 'Antigüedad'}</dt><dd><strong>{milestones.tenureLabel}</strong><small>{employment.endDate ? 'Hasta la fecha de salida' : 'En este empleo'}</small></dd></div>
    {anniversary && <div><dt>{anniversary.daysUntil === 0 ? 'Aniversario hoy' : 'Próximo aniversario'}</dt><dd><strong>Cumplís {anniversary.years} {anniversary.years === 1 ? 'año' : 'años'}</strong><small><time dateTime={anniversary.date}>{dateLabel(anniversary.date)}</time>{anniversary.daysUntil > 0 && ` · ${anniversary.daysUntil === 1 ? 'Mañana' : `Faltan ${anniversary.daysUntil} días`}`}</small></dd></div>}
  </dl>;
}
...
function Employments({ selectedLocation, onNavigate, runSensitive, primaryCountryCode }: { selectedLocation: OwnerLocation; onNavigate: NavigateApp; runSensitive: RunSensitive; primaryCountryCode?: string | null }) {
  ...
  }, [confirmEmploymentDiscard]);

  useEffect(() => {
    if (!editing && !confirmation) return;
    const currentUrl = `${window.location.pathname}${window.location.search}${window.location.hash}`;
    const protectRefresh = (event: BeforeUnloadEvent) => {
      if (savingRef.current || confirming || employmentFormDirty()) { event.preventDefault(); event.returnValue = ''; }
    };
    const protectHistory = () => {
      if (savingRef.current || confirming || !confirmEmploymentDiscard()) {
        window.history.pushState(window.history.state, '', currentUrl);
        return;
      }
  ...
  }, [confirmation, confirming, editing, employmentFormDirty, confirmEmploymentDiscard]);
  ...
  const filteredItems = items.filter((item) => (statusFilter === 'ALL' || item.status === statusFilter) && salaryContextMatches({
    employerName: item.employerName, isFavorite: item.isFavorite, state: 'CONFIRMED', currencyCode: item.currencyCode,
    employmentStatus: item.status, startDate: item.startDate, endDate: item.endDate,
  }, query, item)).sort((a, b) => b.startDate.localeCompare(a.startDate) || Number(b.isFavorite) - Number(a.isFavorite) || a.id.localeCompare(b.id));
  const employmentGroups = [
    { status: 'ACTIVE', label: 'Actuales' }, { status: 'ENDED', label: 'Anteriores' }, { status: 'UNKNOWN', label: 'Por confirmar' },
  ].map((group) => ({ ...group, items: filteredItems.filter((item) => item.status === group.status) }));
  return (
    <div className="page" aria-busy={loading || confirming || saving}>
      ...
      <p className="page-intro">Cada etapa de tu carrera, con sus fechas, antigüedad y sueldos.</p>
      ...
      <div className="stack-form">
        ...
        <section className="employment-list" aria-label="Empleos registrados">
          {items.length > 0 && <>
            <div className="employment-toolbar" role="search" aria-label="Filtrar empleos">
              <label>Buscar empleos<input type="search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Empresa, puesto o año" maxLength={100} autoComplete="off" /></label>
              <label>Estado<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value)}><option value="ALL">Todos</option><option value="ACTIVE">Activos</option><option value="ENDED">Finalizados</option><option value="UNKNOWN">Sin confirmar</option></select></label>
              <div className="employment-results"><span role="status" aria-live="polite">{filteredItems.length} de {items.length} empleo{items.length === 1 ? '' : 's'}</span>{(query || statusFilter !== 'ALL') && <button className="text-button" onClick={() => { setQuery(''); setStatusFilter('ALL'); }}>Limpiar filtros</button>}</div>
            </div>
            {employmentGroups.map((group) => group.items.length > 0 && <section className="employment-group" key={group.status} aria-labelledby={`employment-group-${group.status}`}>
              <div className="employment-group-heading"><h2 id={`employment-group-${group.status}`}>{group.label}</h2><span>{group.items.length}</span></div>
              <div className="employment-rows">{group.items.map((item) => {
                ...
                return <article className="employment-card interactive employment-row" data-status={item.status} key={item.id}>
                  <a className="employment-card-link" href={`/${writeOwnerLocation('', { section: 'jobs', ...itemLocation })}`} aria-label={`Abrir detalle de ${item.employerName}, ${item.role || 'puesto sin especificar'}, desde ${dateLabel(item.startDate)}`} onClick={(event) => { if (!event.ctrlKey && !event.metaKey && !event.shiftKey && !event.altKey) { event.preventDefault(); onNavigate('jobs', itemLocation); } }} />
                  <div className="employment-identity"><div className="employer-avatar">{item.employerName.slice(0, 2).toUpperCase()}</div><div>
                    <h3>{item.employerName}</h3>{item.role && <p className="employment-role">{item.role}</p>}
                    <p className="employment-dates"><time dateTime={item.startDate}>{dateLabel(item.startDate)}</time><span aria-hidden="true"> — </span><span className="sr-only"> hasta </span>{item.endDate ? <time dateTime={item.endDate}>{dateLabel(item.endDate)}</time> : item.status === 'ACTIVE' ? 'Actualidad' : 'Fin sin confirmar'}</p>
                    <div className="employment-badges">{item.isFavorite && <span className="employment-favorite">★ Favorita</span>}{item.employerStatus === 'PENDING' && <span className="status pending">Empresa por verificar</span>}</div>
                  </div></div>
                  <div className="employment-timing"><EmploymentMilestones employment={item} />{item.status === 'UNKNOWN' && !item.endDate && <p className="muted">Continuidad sin confirmar</p>}</div>
                  <div className="employment-card-summary">{itemScope?.current ? <><span>Último sueldo mensual · {periodLabel(itemScope.current.period)}</span><dl className="employment-salary"><div><dt>Bruto</dt><dd><MoneyValue value={itemScope.current.amounts.grossAmount} currency={itemContext?.currencyCode ?? item.currencyCode} kind="salary" /></dd></div><div><dt>Neto</dt><dd><MoneyValue value={itemScope.current.amounts.netAmount} currency={itemContext?.currencyCode ?? item.currencyCode} kind="salary" /></dd></div></dl><small>{itemScope.evolution.length} período{itemScope.evolution.length === 1 ? '' : 's'} con recibos · {documentCount} documento{documentCount === 1 ? '' : 's'}{itemScope.coverage.possibleMissingPeriods.length ? ` · ${itemScope.coverage.possibleMissingPeriods.length} posible${itemScope.coverage.possibleMissingPeriods.length === 1 ? '' : 's'} faltante${itemScope.coverage.possibleMissingPeriods.length === 1 ? '' : 's'}` : ''}</small></> : <span>{loading ? 'Cargando historial…' : !salaryHistory ? 'Historial no disponible' : 'Sin sueldo mensual registrado'} · {item.currencyCode}</span>}</div>
                  <details className="employment-menu"><summary aria-label={`Acciones para ${item.employerName}, desde ${dateLabel(item.startDate)}`}>•••</summary><div><button type="button" className="text-button" disabled={saving} onClick={() => void toggleFavorite(item)}>{item.isFavorite ? 'Quitar de favoritas' : 'Marcar favorita'}</button><button className="text-button" disabled={saving} onClick={() => setEditing(item)}>Editar</button><button className="text-button danger-text" disabled={saving} onClick={() => void remove(item)}>Eliminar</button></div></details>
                </article>;
              })}</div>
            </section>)}
            {!filteredItems.length && <EmptyState title="No encontramos empleos" body="Probá con otra empresa, puesto, año o estado." />}
          </>}
          {!items.length && !loading && !error && <EmptyState title={detections.length ? 'Todavía no confirmaste empleos' : 'Sumá tu primer empleo'} body={detections.length ? 'Confirmá una detección o agregá un empleo manualmente.' : 'Podés empezar por tu trabajo actual y completar el resto después.'} action={<button className="button primary" disabled={saving} onClick={() => setEditing('new')}>Agregar empleo</button>} />}
        </section>
      </div>
      ...
      {confirmation && <ResponsiveDialog labelId="employment-confirmation-title" busy={confirming} onClose={closeConfirmation}>
        <section className="modal" aria-describedby="employment-confirmation-description">
          ...
          <form ref={employmentForm} className="stack-form" onSubmit={confirmDetection}>
...
function Importer({ onBusyChange, onDone }: { onBusyChange: (busy: boolean) => void; onDone: () => void }) {
  ...
  return (
    <div className="page">
      ...
      <label className={`drop-zone${hasActiveBatch || busy ? ' disabled' : ''}`} aria-disabled={hasActiveBatch || busy} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); choose(event.dataTransfer.files); }}><input type="file" accept="application/pdf,.pdf" multiple disabled={hasActiveBatch || busy} onChange={(event) => choose(event.target.files)} /><span className="upload-mark">↑</span><strong>{hasActiveBatch ? 'Hay un lote en curso' : 'Elegí tus recibos PDF'}</strong><span>{hasActiveBatch ? 'Cuando termine vas a poder iniciar otro' : 'Tocá para seleccionar uno o varios archivos'}</span><small>También podés arrastrarlos desde tu computadora. Sólo se admiten recibos PDF.</small></label>
      {error && <p className="message error" role="alert">{error}</p>}
...
type DocumentFilterValues = {
  ...
  period: string;
  settlementType: string;
  status: (typeof documentStatusGroups)[number][0];
};

function DocumentFilters({ kind, onApply, ...values }: DocumentFilterValues & {
  kind: 'ALL' | 'PAYROLL' | 'UNSUPPORTED';
  onApply: (values: DocumentFilterValues) => void;
}) {
  const panel = useRef<HTMLDetailsElement>(null);
  const form = useRef<HTMLFormElement>(null);
  const count = [values.search, values.year !== 'all', values.period, values.settlementType !== 'all', values.status !== 'ALL'].filter(Boolean).length;
  useEffect(() => {
    const desktop = window.matchMedia('(min-width: 1024px)');
    const sync = () => { if (panel.current) panel.current.open = desktop.matches; };
    sync(); desktop.addEventListener('change', sync);
    return () => desktop.removeEventListener('change', sync);
  }, []);
  function close() {
    if (panel.current && !window.matchMedia('(min-width: 1024px)').matches) {
      panel.current.open = false;
      panel.current.querySelector('summary')?.focus();
    }
  }
  return <details ref={panel} className="filter-panel">
    <summary>Filtros{count > 0 ? ` (${count})` : ''}</summary>
    <form ref={form} key={JSON.stringify(values)} className="document-filters" role="search" aria-label="Filtrar documentos" onSubmit={(event) => {
      event.preventDefault();
      const data = new FormData(event.currentTarget);
      onApply({ search: String(data.get('search') || '').trim(), year: String(data.get('year') || 'all'), period: String(data.get('period') || ''), settlementType: String(data.get('settlementType') || 'all'), status: String(data.get('status') || 'ALL') as DocumentFilterValues['status'] });
      close();
    }}>
      <label>Buscar<input name="search" type="search" defaultValue={values.search} maxLength={100} placeholder="Archivo, empresa, mes o tipo" autoComplete="off" /></label>
      <label>Año<input name="year" type="text" inputMode="numeric" pattern="20[0-9]{2}" maxLength={4} defaultValue={values.year === 'all' ? '' : values.year} placeholder="Todos" title="Ingresá un año entre 2000 y 2099" /></label>
      <label>Período<input name="period" type="month" defaultValue={values.period} min="2000-01" max="2099-12" /></label>
      {kind !== 'UNSUPPORTED' && <label>Tipo de liquidación<select name="settlementType" defaultValue={values.settlementType}><option value="all">Todos</option>{settlementTypeOptions.map((value) => <option value={value} key={value}>{settlementTypeLabel(value)}</option>)}</select></label>}
      <label>Estado<select name="status" defaultValue={values.status}>{documentStatusGroups.map(([value, label]) => <option value={value} key={value}>{label}</option>)}</select></label>
      <div className="filter-actions"><button type="submit" className="button primary">Aplicar</button><button type="button" className="button secondary" onClick={() => { onApply({ search: '', year: 'all', period: '', settlementType: 'all', status: 'ALL' }); form.current?.reset(); close(); }}>Limpiar</button><button type="button" className="button secondary filter-cancel" onClick={() => { form.current?.reset(); close(); }}>Cancelar</button></div>
    </form>
  </details>;
}

function History({ initialLocation, onLocationChange, onNavigate, runSensitive }: { initialLocation: OwnerLocation; onLocationChange: (patch: OwnerLocationPatch, replace?: boolean, preserveDocument?: boolean) => void; onNavigate: NavigateApp; runSensitive: RunSensitive }) {
  const { enabled: privacyEnabled } = usePrivacyMode();
  ...
  useEffect(() => {
    const active = document.getElementById(`history-tab-${tab}`);
    const list = active?.parentElement;
    if (active && list) list.scrollLeft = active.offsetLeft - list.offsetLeft;
  }, [tab]);
  const [selectedScopeKey, setSelectedScopeKey] = useState('');
  ...
  useEffect(() => {
    ...
    return () => { stopped = true; };
  }, [detailReload, selectedId, selectedStatus]);
  ...
  function selectScope(key: string) {
    ...
    setDocumentYear('all'); setDocumentPeriod('');
    setDocumentEmploymentId(nextContext?.employmentId ?? 'all');
  ...
  function selectHistoryTab(next: HistoryTab) {
    ...
    if (next === 'documents') {
      const year = tab === 'annual' || tab === 'concepts' ? selectedYear : documentYear;
      const period = tab === 'evolution' || tab === 'purchasing-power' ? selectedPeriod : documentPeriod;
      setDocumentYear(year); setDocumentPeriod(period);
  ...
  function showPeriodDocuments(period: string) {
    setTab('documents'); setDocumentPeriod(period); setDocumentYear('all'); setDocumentEmploymentId(context?.employmentId ?? 'all'); setCheckedDocumentIds([]); setSelected(null);
  ...
  function selectDocumentKind(kind: (typeof documentKinds)[number][0]) {
    setDocumentKind(kind); setDocumentYear('all'); setDocumentPeriod('');
  ...
  return (
    <div className="page" aria-busy={loading || documentsLoading || comparisonLoading || conceptLoading || conceptLoadingMore}>
      ...
      {evolutionTab && <section id={`history-panel-${tab}`} role="tabpanel" aria-labelledby={`history-tab-${tab}`} tabIndex={0}>{scope ? <section className="panel chart-panel"><div className="panel-heading"><div><p className="eyebrow">{tab === 'purchasing-power' ? 'Poder adquisitivo' : 'Evolución'}</p><h2>{selectedPerspective === 'nominal' ? 'Evolución del sueldo habitual' : `Sueldo habitual · ${selectedPerspectiveLabel}`}</h2></div></div>{selectedPerspective !== 'nominal' && <div className="economic-context" role="note"><strong>Contexto económico</strong><span>{context?.countryCode ?? 'País sin confirmar'} · moneda original {scope.currencyCode}{history?.economicCalculationVersion ? ` · cálculo ${history.economicCalculationVersion}` : ''}</span><p>{economicContextDescription}</p></div>}<SalaryEvolution scope={scope} perspective={selectedPerspective} limit={evolutionRanges.find(([value]) => value === selectedEvolutionRange)?.[2]} recoveryPeriods={candidatePeriods} selectedPeriod={selectedPoint?.period} onSelectPeriod={selectEvolutionPeriod} />{selectedPoint && <section className="settlement-detail" aria-labelledby="selected-period-title"><div className="panel-heading"><div><p className="eyebrow">Período seleccionado · {selectedPerspectiveLabel}</p><h3 id="selected-period-title">{periodLabel(selectedPoint.period)}</h3></div><button type="button" className="icon-button" aria-label="Cerrar detalle del período" onClick={() => { setSelectedPeriod(''); onLocationChange({ period: null }, true); }}>×</button></div>{selectedPerspective !== 'nominal' && <div className="economic-period-state"><span className={`status ${economicStatusClass(selectedEconomicProjection?.status ?? 'UNAVAILABLE')}`}>{economicStatusLabels[selectedEconomicProjection?.status ?? 'UNAVAILABLE']}</span><p>{economicStatusMessage(selectedEconomicProjection?.status ?? 'UNAVAILABLE', selectedEconomicProjection?.reason ?? null)}</p></div>}<dl className="settlement-overview"><div><dt>Básico comparable</dt><dd><MoneyValue value={selectedPointComparable} currency={selectedPointCurrency} kind="salary" /></dd></div><div><dt>Bruto total del mes</dt><dd><MoneyValue value={selectedPointAmounts?.grossAmount} currency={selectedPointCurrency} kind="salary" /></dd></div><div><dt>Neto total del mes</dt><dd><MoneyValue value={selectedPointAmounts?.netAmount} currency={selectedPointCurrency} kind="salary" /></dd></div><div><dt>Descuentos / créditos</dt><dd><MoneyValue value={selectedPointAmounts?.deductionsAmount} currency={selectedPointCurrency} kind="salary" creditAware /></dd></div></dl>{selectedPerspective !== 'nominal' && <EconomicEvidence observations={selectedEconomicObservations} referencePeriod={selectedEconomicProjection?.referencePeriod} />}<div className="modal-actions"><button type="button" className="button primary" onClick={() => showPeriodDocuments(selectedPoint.period)}>Ver conceptos y documentos fuente</button>{periods.length > 1 && <button type="button" className="button secondary" onClick={() => preparePeriodComparison(selectedPoint.period)}>Comparar período</button>}</div></section>}</section> : history && !loading ? <EmptyState title="Sin evolución" body="Todavía no hay liquidaciones analizadas." /> : null}</section>}
      ...
      {tab === 'documents' && <section id="history-panel-documents" role="tabpanel" aria-labelledby="history-tab-documents" tabIndex={0}>
        ...
        <DocumentFilters kind={documentKind} search={documentSearch} year={documentYear} period={documentPeriod} settlementType={documentSettlementType} status={documentStatusGroup} onApply={(filters) => {
          setDocumentSearch(filters.search); setDocumentYear(filters.year); setDocumentPeriod(filters.period);
          setDocumentSettlementType(filters.settlementType); setDocumentStatusGroup(filters.status);
      ...
      {selected && (!detail || detailError) && <ResponsiveDialog labelId="document-loading-title" busy={reviewBusy} onClose={closeDetailState}><section className="modal"><div className="modal-head"><div><p className="eyebrow">Documento privado</p><h2 id="document-loading-title">{documentName(selected, privacyEnabled)}</h2></div><button className="icon-button" disabled={reviewBusy} onClick={closeDetailState} aria-label="Cerrar">×</button></div>{detailError ? <><p className="message error" role="alert">{detailError}</p><div className="modal-actions"><button type="button" className="button secondary" disabled={reviewBusy} onClick={closeDetailState}>Cerrar</button><button type="button" className="button primary" disabled={reviewBusy} onClick={() => setDetailReload((value) => value + 1)}>Reintentar</button></div></> : <p aria-live="polite">Cargando metadatos y datos extraídos…</p>}</section></ResponsiveDialog>}
      {selected && detail && !detailError && <Suspense fallback={<ResponsiveDialog labelId="review-loading-title" onClose={closeDetailState}><section className="modal" role="status"><p id="review-loading-title">Cargando revisión…</p><button type="button" className="button secondary" onClick={closeDetailState}>Volver a documentos</button></section></ResponsiveDialog>}><DocumentReview
        key={selected.id}
        detail={detail}
...
function MfaSettings({ mfaEnabled, onSessionsChanged, onUserChanged, runSensitive }: { mfaEnabled: boolean; onSessionsChanged: () => void; onUserChanged: (user: User) => void; runSensitive: RunSensitive }) {
  ...
  return (
    <section className="settings-card setting-wide"><div className="setting-icon" aria-hidden="true">2</div><div><div className="setting-heading"><h2>Segundo factor</h2>{status && <span className={`status ${status.enabled ? 'ready' : 'pending'}`}>{status.enabled ? 'Activo' : 'Inactivo'}</span>}</div>
      {!status && !error && <div className="mfa-loading"><span className="sr-only" role="status">Cargando estado del segundo factor…</span><div aria-hidden="true"><p className={`loading-skeleton ${mfaEnabled ? 'loading-description' : 'loading-note'}`}>&nbsp;</p>{mfaEnabled ? <><dl className="security-details">{Array.from({ length: 3 }, (_, index) => <div key={index}><dt className="loading-skeleton">&nbsp;</dt><dd className="loading-skeleton">&nbsp;</dd></div>)}</dl><p className="setting-note loading-skeleton loading-note">&nbsp;</p><div className="setting-actions inline-actions"><span className="loading-skeleton loading-action">&nbsp;</span><span className="loading-skeleton loading-action">&nbsp;</span></div></> : <div className="stack-form"><p className="loading-skeleton loading-description">&nbsp;</p><span className="loading-skeleton loading-action">&nbsp;</span></div>}</div></div>}
...
function SessionsSettings({ refreshKey, runSensitive }: { refreshKey: number; runSensitive: RunSensitive }) {
  ...
  return <section className="settings-card setting-wide sessions-card"><div className="setting-icon" aria-hidden="true">↪</div><div><div className="setting-heading"><h2>Sesiones activas</h2>{!loading && !error && <span className="status">{sessions.length}</span>}</div><p>Revisá en qué navegadores y dispositivos está abierta tu cuenta. No usamos ubicación ni fingerprinting para identificarlos.</p>
