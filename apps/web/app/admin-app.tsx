...
async function api<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    if (body.error?.code === 'AUTHENTICATION_REQUIRED') window.dispatchEvent(new Event('salarivo:session-expired'));
    throw new ApiError(body.error?.message ?? 'No pudimos completar la operación.', response.status, body.error?.code ?? 'REQUEST_FAILED');
  }
...
function QueryFilters({ action, search, children, searchPlaceholder = 'UUID exacto' }: { action: string; search: URLSearchParams; children?: ReactNode; searchPlaceholder?: string }) {
  const activeFilters = Array.from(search).filter(([key, value]) => value && !['page', 'pageSize', 'healthPage'].includes(key)).length;
  const panel = useRef<HTMLDetailsElement>(null);
  useEffect(() => {
    if (panel.current && window.matchMedia('(min-width: 1024px)').matches) panel.current.open = true;
  }, []);
  return <details ref={panel} className="admin-filter-panel" open={activeFilters > 0}>
    <summary>Filtros{activeFilters > 0 ? ` (${activeFilters})` : ''}</summary>
    <form className="admin-filters" action={action} method="get" role="search" onSubmit={(event) => {
    ...
    <label>Buscar<input name="search" type="search" autoComplete="off" autoCapitalize="none" spellCheck={false} defaultValue={search.get('search') ?? ''} placeholder={searchPlaceholder} /></label>
    {children}
    <button className="button secondary" type="submit">Aplicar filtros</button>
    {activeFilters > 0 && <a className="text-button" href={action}>Limpiar</a>}
    <button type="button" className="text-button" onClick={(event) => { event.currentTarget.form?.reset(); if (panel.current) { panel.current.open = false; panel.current.querySelector('summary')?.focus(); } }}>Cancelar</button>
  </form></details>;
}
...
function ActionDialog({ action, onClose, onDone }: { action: AdminAction; onClose: () => void; onDone: (message: string) => void }) {
  ...
  return <dialog className="admin-dialog" ref={ref} onCancel={(event) => { if (busy) event.preventDefault(); else onClose(); }} onClose={onClose} aria-labelledby="admin-dialog-title" aria-describedby="admin-dialog-description">
    <form onSubmit={submit} aria-busy={busy}>
      <div className="modal-head"><div><p className="eyebrow">Acción auditada</p><h2 id="admin-dialog-title">{stepUp ? 'Confirmá tu identidad' : action.title}</h2></div><button type="button" className="icon-button" disabled={busy} onClick={onClose} aria-label="Cerrar">×</button></div>
      ...
      {stepUp ? <label>Código MFA<input name="code" inputMode="numeric" autoComplete="one-time-code" pattern="[0-9]{6}" minLength={6} maxLength={6} required autoFocus /></label> : <>
        ...
        <label>Referencia o ticket<input name="reference" autoComplete="off" autoCapitalize="none" spellCheck={false} required minLength={3} maxLength={80} pattern="[A-Za-z0-9][A-Za-z0-9._:/-]*" placeholder="SUP-1234" /></label>
      </>}
...
function AdminLayout({ user, path, children }: { user: SessionUser; path: string; children: ReactNode }) {
  ...
  return <div className="admin-shell"><AppNavigation id="admin-navigation" className="admin-sidebar" open={menuOpen} onClose={closeMenu} label="Menú administrativo"><div className="admin-sidebar-head"><Link className="brand" href="/"><span className="brand-mark">S</span>Salarivo</Link><button className="icon-button admin-menu-close" onClick={closeMenu} aria-label="Cerrar menú">×</button></div><div className="admin-context"><small>Consola interna</small><strong>{user.adminRole}</strong></div><nav aria-label="Navegación administrativa">{allowed.map((item) => { const active = item.href === '/admin' ? path === '/admin' : path.startsWith(item.href); return <a key={item.href} className={active ? 'admin-nav active' : 'admin-nav'} aria-current={active ? 'page' : undefined} href={item.href} onClick={closeMenu}><span aria-hidden="true">{item.mark}</span>{item.label}</a>; })}</nav><div className="admin-identity"><strong>{user.displayName || 'Administrador'}</strong><small>{user.email}</small><Link href="/">Volver al espacio personal</Link></div></AppNavigation><main className="admin-content" id="admin-main"><header className="admin-mobile-header"><button className="icon-button" onClick={() => setMenuOpen(true)} aria-label="Abrir menú" aria-expanded={menuOpen} aria-controls="admin-navigation">☰</button><span>Administración<span className="admin-mobile-role"> · {user.adminRole}</span></span><Link href="/" aria-label="Volver a Salarivo">S</Link></header><div className="admin-page-content" key={path}>{children}</div></main></div>;
}
...
export function AdminApp() {
  ...
  useEffect(() => {
    ...
    function expireSession() {
      expired = true;
      setUser(null);
      setError('Tu sesión terminó. Iniciá sesión nuevamente para acceder al panel.');
    }
    async function checkSession(initial = false) {
      if (pending || expired || (!initial && document.visibilityState === 'hidden')) return;
      pending = true;
      try {
        const current = await api<SessionUser>('/auth/me');
        if (!active || expired) return;
        if (current.role !== 'ADMIN' || !current.adminRole) {
          setUser(null);
        ...
        if (active && !expired && initial) setError(caught instanceof Error ? caught.message : 'No pudimos abrir el panel.');
      } finally { pending = false; }
    }
    const revalidate = () => { void checkSession(); };
    window.addEventListener('salarivo:session-expired', expireSession);
    ...
    return () => {
      active = false;
      window.removeEventListener('salarivo:session-expired', expireSession);
