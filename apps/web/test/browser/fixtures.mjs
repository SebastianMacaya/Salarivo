...
const employment = { id: id(10), employerId: id(20), employerName, role: 'Especialista en sistemas y operaciones de prueba', startDate: '2025-01-01', endDate: null, status: 'ACTIVE', countryCode: 'AR', currencyCode: 'ARS', isFavorite: true, employerStatus: 'VERIFIED' };
const user = { id: id(1), email: 'persona.sintetica@example.test', displayName: 'Persona Sintética', role: 'USER', adminRole: null, permissions: [], authState: 'AUTHENTICATED', mfaEnabled: true, onboardingCompleted: true, legalAcceptanceRequired: false, authMethods: ['GOOGLE'] };
...
function installFixture(data, options) {
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (/^\/documents\/[^/]+$/.test(path)) return ok({ ...data.detail, id: path.split('/')[2] });
