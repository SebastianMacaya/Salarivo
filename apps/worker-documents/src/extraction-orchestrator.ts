...
import type { ApprovedDocumentLayout } from '@salarivo/database/document-layouts';
...
type LayoutResolver = (text: string, employerName: string) => Promise<ApprovedDocumentLayout | null>;
...
async function parse(input: ExtractionText, pageCount: number, resolveLayout?: LayoutResolver): Promise<{
  extraction: PayrollExtraction; layout: ExtractionLayout;
}> {
  const initial = extractArgentinePayroll(input.text, input.source);
  const fingerprint = fingerprintLayout(input.text, pageCount);
  const resolved = fingerprint && initial.employerName && resolveLayout
    ? await resolveLayout(input.text, initial.employerName) : null;
  const profile = resolved?.fingerprint === fingerprint && resolved.fingerprintVersion === layoutFingerprintVersion ? resolved : null;
  const extraction = profile ? extractArgentinePayroll(input.text, input.source, profile.aliases) : initial;
  return { extraction: attachSpatialEvidence(extraction, input.evidence),
...
export async function orchestrateExtraction(
  ...
  const initialParsed = await parse(input, pageCount, options.resolveLayout);
  const initial = initialParsed.extraction;
  const reason = options.requiredReason ?? ocrFallbackReason(input.text, initial);
  const outcome: ExtractionOutcome = { ...input, extraction: initial, issues: [], triggerReason: reason, layout: initialParsed.layout };
  ...
  try {
    ...
    return { ...outcome, issues: [error.code] };
  }
  const text = result.text;
  const evidence = ocrEvidence(result);
  const candidateParsed = await parse({ text, evidence, source: 'OCR' }, pageCount, options.resolveLayout);
  ...
  return { text, evidence, source: 'OCR', extraction: candidate, issues, ocrResult: result, triggerReason: reason, layout: candidateParsed.layout };
}
